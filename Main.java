...
//									 true(증거가 있을 때)
			                     ...
			                     //추가 정보
			                     if (dao.checkClue(4, id) == 4 || dao.checkClue(14, id) == 14) {
			                        
			                     System.out.println(nick + " : " + "상민씨 추가로 나온 증거가 있습니다! "
...
//							 true(증거가 있을 때)
							...
							System.out.println(nick + " : 박나영 원장에게 질문을 한다.");
		                     ...
		                     System.out.println("나영 : 좀 건방지긴 하지만 일은 잘했어요. 그래서 승진도 빨랐구요. \n"
		                           + "유능한 직원을 홧김에 죽이는 건 멍청이들이나 하는 짓이죠. 멍청하지 않다면 제가 죽였을 거라고 생각하진 않으시겠죠?!");
		                     
		                     //추가 정보
		                     if (dao.checkClue(5, id) == 5 || dao.checkClue(8, id) == 8 || dao.checkClue(10, id) == 10) {

		                     System.out.println(nick + " : 지금까지 알아본 바로는 최근에 나영씨와 pbk 사이에 좀 큰 문제가 있었는데, 그것과는, ");
		                     System.out.println("나영 : 아니!! 저는 계속 원장실에 있었어요! 중요한 프로젝트가 있어서 9시 15분에서 10시 20분까지 여기서 시청공무원과 회의중이었다구요!");
		                     System.out.println("나영 : 이건 시간 낭비야! 차라리 장현도 경비원을 의심하라구요!\n"
		                           + "휴... 여튼, 경비원 그 사람 PBK랑 수시로 같이 있는데다가, 빚도 있는 것 같던데? 분명 뭔가 있었어요. 남들은 속아도 나는 보여요.");
		                     }
...
//						 true(증거가 있을 때)
							...
							if (dao.checkClue(6, id) == 6 || dao.checkClue(11, id) == 11) {
							   ...
							   System.out.println(nick + " : " + "(덩치가 커서... 좀 쫄았다..)... 네 알겠습니다. (하지만 친하기만 한 사이는 아닐지도 몰라..)");
...
//							 true(증거가 있을 때)
							...
							if (dao.checkClue(7, id) == 7 || dao.checkClue(12, id) == 12) {
							                     // 증거나 증언이 있을때 추가로 얻을 수 있는 심적증거
							System.out.println(nick + " : " + "혹시 하라 선생님은 흡연을 하시나요?");
							System.out.println("하라 : 아니요 전 담배 냄새도 싫어서 담배 근처도 안가는 사람이에요."
							               + "학생들이랑 같은 공간에 있어야 하는데 안좋은 냄새를 풍기면 안돼죠.");
							System.out.println("하라 : 아! 아까 장현도씨 물어보셔서 말씀드리는데, 그분 PBK랑 엄청 친해요.아마 여기 일도...");
							System.out.println(nick + " : " + "혹시 얼마나 친한지 아시나요?");
							System.out.println("하라 : 그건 저도 모르죠 현도씨한테 물어봐보시죠?");
							                     }
...
//							 true(증거가 있을 때)
							...
							if (dao.checkClue(3, id) == 3 || dao.checkClue(9, id) == 9 || dao.checkClue(13, id) == 13) {
			                     // 증거나 증언이 있을때 추가로 얻을 수 있는 심적증거
			                     System.out.println("지은 : 형사님 그런데 교무실에 왔을때 옥상 열쇠가 없어져 있었어요.");//
			                     System.out.println(nick + " : " + "옥상 열쇠는 지은씨가 관리하시는 건가요?");
			                     System.out.println("지은 : 제가 관리하는건 아니고 당직 선생님 매일 관리 한답니다. 항상 옥상 열쇠는 보관함에 있었는데 보니깐 없었어요.");
